...
export class ProfileManager implements vscode.Disposable {
    ...
    public getProfilesMetadata(): { name: string, textGroupCount: number, regexGroupCount: number }[] {
        ...
        const metadata = profiles.map(p => {
            let textGroupCount = 0, regexGroupCount = 0;
            if (p.groups) {
                p.groups.forEach(g => {
                    if (g.isRegex) {
                        regexGroupCount++;
            ...
            return { name: p.name, textGroupCount, regexGroupCount };
        });

        if (!hasDefault) {
            // Default profile usually has Presets (1 group)
            metadata.unshift({ name: Constants.Labels.DefaultProfile, textGroupCount: 1, regexGroupCount: 0 });
        }
